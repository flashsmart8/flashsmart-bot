...
SYSTEM_PROMPT = """Ти — AI менеджер FlashSmart. Категорії: Захисне скло, Кабелі, МЗП, Навушники.
Спілкуйся мовою клієнта (укр/рос/англ). Будь живим і професійним.

ПРАВИЛА — відповідай ТІЛЬКИ JSON коли потрібна дія:
Запит про категорію/товар → {"action":"list_products","category":"Навушники"}
Конкретний товар → {"action":"check_stock","product":"назва"}
Замовлення → {"action":"create_order","items":[{"name":"назва","qty":10}]}
Перевірити борг → {"action":"check_debt"}

Категорії: навушники/наушники/headphones → Навушники; кабель/cable/дріт → Кабелі; скло/glass → Захисне скло; зарядка/charger/МЗП/адаптер → МЗП; все/асортимент → all

ЗАБОРОНЕНО: не кажи що товару немає без перевірки. JSON — без тексту навколо.
Після даних зі складу — відповідай живо, уточнюй деталі."""
...
async def execute_action(action_data, db, user_id, client_rec):
    ...
    if action == "list_products":
        ...
        for req in items_req:
            name_q = req.get("name","").lower()
            qty = int(req.get("qty",1))
            matches = [p for p in products if name_q in p.get("name","").lower() or p.get("name","").lower() in name_q]
            if not matches:
        ...
        # Build response
        if not_found:
            return f"❌ Товари не знайдено: {', '.join(not_found)}\n\nПеревірте назву і спробуйте ще раз."
        if no_stock:
            return f"⚠️ Недостатньо на складі:\n" + "\n".join(no_stock)
        if not order_items:
            return "Не вдалося сформувати замовлення."
        ...
        for item in order_items:
            result += f"• {item['name']} × {item['qty']} шт = ${item['sum']:.2f}\n"
        result += f"\n💰 *Разом: ${total:.2f}*"
        result += f"\n_Рівень: {level_labels.get(price_level,'Дилер 2')}_"
        result += "\n\nПідтвердіть замовлення — напишіть *підтверджую* або *скасувати*"
        return result
