...
def main_menu_keyboard():
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🛒 Відкрити магазин", web_app=WebAppInfo(url=SHOP_URL))],
        [
            InlineKeyboardButton("📦 Каталог", callback_data="menu_catalog"),
            InlineKeyboardButton("💰 Борг", callback_data="menu_debt"),
        ],
        [
            InlineKeyboardButton("📋 Мої замовлення", callback_data="menu_orders"),
            InlineKeyboardButton("💬 Чат з менеджером", callback_data="menu_chat"),
        ]
    ])
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "confirm_order":
            if user_id in pending_orders:
                result = await save_invoice(user_id, db)
                await query.message.reply_text(result, parse_mode="Markdown", reply_markup=main_menu_keyboard())
            else:
                await query.message.reply_text("Немає активного замовлення.", reply_markup=main_menu_keyboard())
                
        elif data == "cancel_order":
            if user_id in pending_orders:
                del pending_orders[user_id]
                await query.message.reply_text("Замовлення скасовано.", reply_markup=main_menu_keyboard())
            else:
                await query.message.reply_text("Немає активного замовлення.")
                
        elif data == "menu_catalog":
            response = await process_message(user_id, "покажи весь асортимент", db, client_rec)
            await safe_reply(query.message, response)
            
        elif data == "menu_debt":
            response = await process_message(user_id, "який у мене борг?", db, client_rec)
            await safe_reply(query.message, response)
            
        elif data == "menu_orders":
            if not client_rec:
                await query.message.reply_text("Спочатку надішліть номер телефону для ідентифікації.")
                return
            invs = [i for i in (db.get("invoices") or []) if i.get("clientId") == client_rec.get("id")]
            invs.sort(key=lambda x: x.get("date",""), reverse=True)
            if not invs:
                await query.message.reply_text("У вас поки немає замовлень.")
                return
            last5 = invs[:5]
            text = "📋 *Ваші останні замовлення:*\n\n"
            for inv in last5:
                num = String_pad(inv.get("num",0))
                d = inv.get("date","")
                total = inv.get("total",0)
                items_count = len(inv.get("items",[]))
                src = "📱" if inv.get("source")=="miniapp" else "🤖" if inv.get("source")=="telegram" else "💻"
                text += f"{src} #{num} від {d} — {items_count} поз. — *${total:.2f}*\n"
            await safe_reply(query.message, text)
            
        elif data == "menu_chat":
            await query.message.reply_text("Пишіть — я на зв'язку! Можу допомогти з вибором товару, оформити замовлення або відповісти на питання. 💬")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Add confirm buttons if there's a pending order
        markup = None
        if user_id in pending_orders:
