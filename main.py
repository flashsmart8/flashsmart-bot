...
async def parse_supplier_price_list(text: str, products: list, incomes: list) -> str:
    """Send forwarded supplier message to Claude AI and return comparison table."""
    # Build map: productId -> last income price
    last_income = {}
    for inc in sorted(incomes, key=lambda x: x.get("date", "")):
        pid = inc.get("productId")
        pr = inc.get("price") or 0
        if pid and pr > 0:
            last_income[str(pid)] = pr

    my_prices_info = ""
    for p in products:
        pid = str(p.get("id", ""))
        pp = last_income.get(pid, 0)
        if pp > 0:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.forward_from or update.message.forward_from_chat or update.message.forward_date:
        ...
        try:
            db = await load_db()
            products = db.get("products") or []
            incomes = db.get("incomes") or []
            result = await parse_supplier_price_list(fwd_text, products, incomes)
